$(function () {
    ...
    $('li.settings').addClass('opened active').find('ul').addClass('visible').find('li.apartments').addClass('active');
    ...
    $table.DataTable({
        ...
        ajax: {
            url: base_url + 'admin/apartment/all',
            type: 'POST'
        },
        columns: [
    ...
    function createUpdate() {
        $.ajax({
            url: $('#form_apartment').attr('action'),
            type: 'POST',
            dataType: 'json',
            data: new FormData($('#form_apartment')[0]),
            processData: false,
            contentType: false,
            success: function(response) {
