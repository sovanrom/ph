$(function () {
    ...
    var $table = $('#actual_stock');
    var $modal = $('#my_modal');

    $('li.stationarys').addClass('opened active').find('ul').addClass('visible').find('li.actual_stocks').addClass('active');
    ...
    $table.DataTable({
        ...
        ordering: false,
        ajax: {
            url: base_url + 'admin/actual_stock/all',
            type: 'POST'
        },
        columns: [
    ...
    $('.main-content').find('a[rel="add"]').hide();

    function createUpdate() {
        $.ajax({
            url: $('#form_actual_stock').attr('action'),
            type: 'POST',
            dataType: 'json',
            data: new FormData($('#form_actual_stock')[0]),
            processData: false,
            contentType: false,
            success: function(response) {
