$(function () {
    ...
    $('li.stationarys').addClass('opened active').find('ul').addClass('visible').find('li.departments').addClass('active');
    ...
    $table.DataTable({
        ...
        ajax: {
            url: base_url + 'admin/department/all',
            type: 'POST'
        },
        columns: [
    ...
    function createUpdate() {
        $.ajax({
            url: $('#form_departments').attr('action'),
            type: 'POST',
            dataType: 'json',
            data: new FormData($('#form_departments')[0]),
            processData: false,
            contentType: false,
            success: function(response) {
